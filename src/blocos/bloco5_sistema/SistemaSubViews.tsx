...
export function GestaoProdutosView() {
  ...
  const rawFilterNecessidades: string[] = filterRubrica === "TODAS"
    ? Array.from(new Set(products.map((p) => p.necessidade).filter((n): n is string => Boolean(n))))
  ...
  const filtered = products.filter((p) => {
    if (filterRubrica === "TODAS") {
      return false;
    }
    
    // Match Rubrica
    const matchRubrica = (p.rubrica || "").trim().toLowerCase() === filterRubrica.trim().toLowerCase();
    if (!matchRubrica) return false;

    // Match Necessity if not "TODAS"
    let matchNecessidade = true;
    ...
    if (!matchNecessidade) return false;
    ...
    const matchSearch = 
      ...
      (p.especificacao || "").toLowerCase().includes(searchQuery.toLowerCase()) ||
      (p.unidade || "").toLowerCase().includes(searchQuery.toLowerCase()) ||
      pCodeNec.toLowerCase().includes(searchQuery.toLowerCase());

    return matchSearch;
  });

  const selectedRubricaNecessidades = getNecessidadesOptions(filterRubrica);
  const displayedNecessidades = filterRubrica === "TODAS"
    ? []
    : filterNecessidade === "TODAS"
      ? selectedRubricaNecessidades
      : selectedRubricaNecessidades.filter((nec) => {
          const formatted = formatNecessidadeWithCode(nec, filterRubrica);
          return (
            nec.trim().toLowerCase() === filterNecessidade.trim().toLowerCase() ||
            formatted.trim().toLowerCase() === filterNecessidade.trim().toLowerCase()
          );
        });

  const handleDeleteProduct = async (nome: string) => {
    if (window.confirm(`Tem certeza que deseja excluir o produto "${nome}" da base de dados?`)) {
      deleteUnifiedProduct(nome);
      const prods = await getUnifiedProducts();
      setProducts(prods);
      setSuccessMsg(`Produto "${nome}" excluído da base de dados com sucesso!`);
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    if (editingProduct._originalNome && editingProduct._originalNome !== editingProduct.nome) {
      deleteUnifiedProduct(editingProduct._originalNome);
    }
    ...
    const prods = await getUnifiedProducts();
    setProducts(prods);
    setSuccessMsg(`Produto "${editingProduct.nome}" atualizado e substituído na base de dados com sucesso! Coerência garantida.`);
  ...
  const handleAddNew = async (e: React.FormEvent) => {
    ...
    const prods = await getUnifiedProducts();
    setProducts(prods);
    setSuccessMsg(`Novo produto "${newProduct.nome}" registado, aplicado e armazenado na base de dados com sucesso!`);
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      <div className="bg-gradient-to-r from-blue-900 to-indigo-900 text-white p-8 rounded-[2rem] shadow-2xl relative overflow-hidden flex flex-col items-center text-center justify-center gap-6">
        <div className="relative z-10 space-y-2 flex flex-col items-center">
          <span className="text-[10px] font-black uppercase tracking-widest bg-blue-500/30 text-blue-200 px-3 py-1 rounded-full border border-blue-400/20 w-fit">
            Administração Central de Preços & Produtos
          </span>
          <h2 className="text-2xl font-black tracking-tight font-serif">
            Gestão Unificada de Produtos
          </h2>
          <p className="text-xs text-blue-200 max-w-2xl leading-relaxed mx-auto">
            Todos os produtos estão agrupados e organizados por Rubrica e Necessidade Específica. Qualquer atualização ou registo de novo produto é aplicado e substituído instantaneamente na base de dados para garantir coerência absoluta em todo o sistema.
          </p>
        </div>
      </div>

      {successMsg && (
        <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 p-4 rounded-2xl text-xs font-bold flex items-center gap-3 shadow-sm">
      ...
      <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold text-gray-800 outline-none focus:border-blue-600"
            />
          </div>

          <div>
            <label className="block text-[10px] font-black text-blue-900 uppercase tracking-widest mb-1">
              Filtrar por Rubrica (Dropdown)
            </label>
            <select
              value={filterRubrica}
              onChange={(e) => {
          ...
          <div>
            <label className="block text-[10px] font-black text-blue-900 uppercase tracking-widest mb-1">
              Filtrar por Necessidade Específica (Dropdown)
            </label>
            <select
            ...
            >
              ...
              {availableFilterNecessidades.map((nec, i) => (
                <option key={i} value={nec}>{nec}</option>
              ))}
            </select>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center pt-2 border-t border-gray-100">
          <span className="text-xs font-bold text-gray-500">
            Produtos organizados e agrupados por Rubrica e Necessidade Específica.
          </span>
          <span className="text-xs font-black text-blue-900 bg-blue-50 px-4 py-2 rounded-xl border border-blue-100">
            Total de Produtos: {filtered.length}
          </span>
        </div>

        {filterRubrica === "TODAS" ? (
          <div className="text-center py-20 bg-blue-50/40 border border-blue-100 rounded-3xl space-y-4">
            <div className="w-12 h-12 bg-blue-100 text-blue-800 rounded-2xl flex items-center justify-center mx-auto font-black text-lg">
              🔍
            </div>
            <div className="space-y-1">
              <h4 className="text-sm font-black text-blue-950 uppercase tracking-wide font-serif">
                Selecione uma Rubrica Orçamental
              </h4>
              <p className="text-xs text-blue-700 max-w-md mx-auto leading-relaxed">
                Por favor, escolha uma Rubrica Orçamental específica no filtro acima para visualizar os cabeçalhos de cada Necessidade e gerir os seus produtos com transparência absoluta.
              </p>
            </div>
          </div>
        ) : displayedNecessidades.length === 0 ? (
          <div className="text-center py-12 text-gray-400 text-xs font-bold">
            Nenhuma necessidade encontrada com os critérios de filtragem selecionados.
          </div>
        ) : (
          <div className="space-y-8">
            {displayedNecessidades.map((necessidadeName) => {
              // Find products belonging to this necessity
              const prodsList = filtered.filter((p) => {
                const pCodeNec = formatNecessidadeWithCode(p.necessidade || "", p.rubrica);
                const cleanP = (p.necessidade || "").trim().toLowerCase();
                const cleanN = necessidadeName.trim().toLowerCase();
                const cleanCodeNec = formatNecessidadeWithCode(necessidadeName, filterRubrica).trim().toLowerCase();
                return cleanP === cleanN || pCodeNec.trim().toLowerCase() === cleanN || pCodeNec.trim().toLowerCase() === cleanCodeNec;
              });

              return (
                <div key={necessidadeName} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-gray-100 pb-4 bg-slate-50/50 p-4 rounded-2xl">
                    <div className="space-y-1">
                      <span className="text-[10px] font-black text-blue-800 bg-blue-100 px-3 py-1 rounded-full border border-blue-200 uppercase tracking-widest">
                        Necessidade Específica
                      </span>
                      <h4 className="text-sm font-black text-gray-900 font-serif leading-snug">
                        {formatNecessidadeWithCode(necessidadeName, filterRubrica)}
                      </h4>
                    </div>
                    ...
                    <button
                      ...
                      onClick={() => {
                        setNewProduct({
                          ...
                          necessidade: necessidadeName,
                        });
                        setIsAddingNew(true);
                      }}
                      className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-black tracking-wider transition-all flex items-center gap-2 shrink-0 shadow-lg shadow-emerald-100 uppercase"
                    >
                      <Plus className="w-4 h-4" /> Registar Produto nesta Necessidade
                    </button>
                  </div>

                  {prodsList.length === 0 ? (
                    <div className="text-center py-8 text-gray-400 text-xs italic">
                      Nenhum produto registado para esta necessidade ainda. Clique no botão acima para registar um produto de forma transparente.
                    </div>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="w-full text-left">
                        <thead className="bg-gray-50/80 text-[10px] font-black text-gray-400 tracking-widest border-b border-gray-100">
                          <tr>
                            <th className="px-4 py-3">Nome do Produto</th>
                            <th className="px-4 py-3">Preço Unificado (MZN)</th>
                            <th className="px-4 py-3">Unidade / Detalhes</th>
                            <th className="px-4 py-3">Especificação</th>
                            <th className="px-4 py-3 text-right">Ações</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-50">
                          {[...prodsList].sort((a: any, b: any) => a.nome.localeCompare(b.nome, "pt-MZ")).map((p: any, idx: number) => (
                            <tr key={idx} className="hover:bg-blue-50/20 transition-colors">
                              <td className="px-4 py-3 font-bold text-gray-900 text-xs font-serif" style={{ letterSpacing: '0.5px' }}>
                                {p.nome}
                              </td>
                              <td className="px-4 py-3 font-mono font-black text-blue-600 text-xs" style={{ letterSpacing: '0.5px' }}>
                                {Number(p.preco || 0).toLocaleString("pt-MZ", { minimumFractionDigits: 2 })} MZN
                              </td>
                              <td className="px-4 py-3 text-xs font-bold text-gray-600" style={{ letterSpacing: '0.5px' }}>
                                {p.unidade || "Unidade"}
                              </td>
                              <td className="px-4 py-3 text-xs text-gray-500 max-w-xs truncate" style={{ letterSpacing: '0.5px' }}>
                                {p.especificacao || "N/A"}
                              </td>
                              <td className="px-4 py-3 text-right">
                                <div className="flex items-center justify-end gap-2">
                                  <button
                                    ...
                                    className="px-3 py-1.5 bg-blue-600 text-white rounded-xl text-[10px] font-black tracking-widest hover:bg-blue-700 transition-all shadow-sm"
                                  >
                                    Atualizar & Substituir
                                  </button>
                                  <button
                                    onClick={() => handleDeleteProduct(p.nome)}
                                    title="Excluir Produto Repetido ou Indesejado"
                                    className="px-3 py-1.5 bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white rounded-xl text-[10px] font-black transition-all border border-rose-200 flex items-center gap-1.5 shadow-sm"
                                  >
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl max-w-lg w-full p-8 space-y-6 border border-gray-100">
            ...
            <form onSubmit={handleSaveEdit} className="space-y-4">
              ...
              <div>
                ...
                >
                  {getNecessidadesOptions(editingProduct.rubrica).map((nec, i) => (
                    <option key={i} value={nec}>{formatNecessidadeWithCode(nec, editingProduct.rubrica)}</option>
                  ))}
                </select>
              </div>
              ...
              <div>
                <label className="block text-[10px] font-black text-blue-900 uppercase tracking-widest mb-1">
                  Unidade / Detalhes (Ex: Litro, Unidade, Kg)
                </label>
                <input
                  type="text"
                  required
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                ...
                >
                  Substituir & Aplicar na Base de Dados
                </button>
              </div>
            </form>
          </div>
      ...
      {/* Modal: Registar Novo Produto */}
      {isAddingNew && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl max-w-lg w-full p-8 space-y-6 border border-gray-100">
            <div className="flex justify-between items-center border-b border-gray-100 pb-4">
              <h3 className="text-base font-black text-blue-950 font-serif">
                Registar Novo Produto na Base de Dados
              </h3>
              <button
            ...
            <form onSubmit={handleAddNew} className="space-y-4">
                ...
                return (
                  <div className="bg-blue-50/80 border border-blue-200 p-4 rounded-2xl space-y-2">
                    <label className="block text-[10px] font-black text-blue-900 uppercase tracking-widest">
                      ✨ Selecionar de Produtos Padrão (Autocompletar)
                    </label>
                    <select
                      onChange={(e) => {
              ...
              <div>
                ...
                >
                  {availableNecessidadesForNew.map((nec, i) => (
                    <option key={i} value={nec}>{formatNecessidadeWithCode(nec, newProduct.rubrica)}</option>
                  ))}
                </select>
              </div>
              ...
              <div>
                <label className="block text-[10px] font-black text-blue-900 uppercase tracking-widest mb-1">
                  Unidade / Detalhes (Ex: Unidade, Caixa, Litro)
                </label>
                <input
                  type="text"
                  required
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                <button
                  type="button"
                  onClick={() => setIsAddingNew(false)}
                  className="px-6 py.5 bg-gray-100 text-gray-600 rounded-xl text-xs font-bold hover:bg-gray-200 transition-colors"
                >
                  Cancelar
                </button>
                <button
                ...
                >
                  Registar & Guardar na Base de Dados
                </button>
              </div>
            </form>
          </div>
